<script lang="ts">
  ...
  const wsQuery = getNotCurrentWorkspace();
  $effect(() => {
    if (wsQuery.current) {
      updateWorkspace.fields.set({name: wsQuery.current.name,});
    }
  });
</script>

{#if wsQuery.loading}
  <Spinner>Loading workspace data</Spinner>
{:else}
  <Card title={wsQuery.current!.name}>
    {#snippet back()}
