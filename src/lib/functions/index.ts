...
export async function testFunctionRole(role: Permission): Promise<ReturnType<typeof getRequestEvent>> {
  const event = getRequestEvent();
  if (!event.locals.user || !event.locals.workspace) {
    invalid("Unauthorized");
  }
  const r = await getRole(event.locals.user.id, event.locals.workspace.id);
